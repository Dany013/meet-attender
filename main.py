...
def ora(k):
    for i in range(
        ...
        if str(lista_ore[i]) == datetime.datetime.now().strftime(
            ...
            y = (
                k + 2
            )  # numero della colonna (+2 perchè la lista parte da 0 e il file excel ha la colonna delle ore da non contare)
...
def meet(materia, k):
    try:
        az = dict_classi[
            materia
        ]  # Questo serve per far vedere le scrite prima di aprire il link di meet (se "materia" non è definita ritora KeyError)
        print(colorama.Fore.GREEN + colorama.Style.BRIGHT + "\n\nMeet found")
        print(colorama.Fore.WHITE + "Connecting to: {}".format(materia))
        print(colorama.Fore.WHITE)
        webbrowser.open(
            dict_classi[materia], autoraise=True
        )  # apro il link alla classe di meet corrispondente all'ora e al giorno
...
print(
    colorama.Fore.WHITE
    + "The program is running.\nThe Meet will open at the indicated time.\n(if it's not started you'll have to reload the page)\n"
)
